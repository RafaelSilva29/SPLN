...
proib = ['I','Erised','We\'ve','Why','Thanks','No','How','All','Come','The','So','Oh','Don\'t','You','He\'s','It','We','He','It\'s','Mr','Mrs','And','Well','What','Yeah','Yes','But','An','Er','That\'s','If','Can\'t','What\'s']
listaPares = otimiza()

#CONSTRUIR GRAFO DE RELACOES
G = nx.Graph()
for w,v in listaPares.items():
    G.add_edge(w[0], w[1], weight=v )

plt.figure(figsize = (15, 15))
nx.draw(G, with_labels = True, node_size = 1500, font_size = 10)
plt.show()

print('------------------------- INSIRA UMA PERSONAGEM: -------------------------')
while True:
    line = input()
    if line in list(G.nodes):
        print('Número de Amigos: ', numeroAmigos(G,line))
        print('Lista dos 5 Amigos mais próximos:')
        for a,n in topAmigos(G,line,5).items():
            print(a)
        showGrafoIndividual(G,line)
    else:
